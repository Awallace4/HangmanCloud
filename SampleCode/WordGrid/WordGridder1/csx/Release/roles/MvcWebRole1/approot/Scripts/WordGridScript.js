...
// Global constants
var BoardSize = 17;
var CenterSquare = 8 * BoardSize + 8;
var MaxTilesInRack = 8;
...
var currentRow = BoardSize / 2
var currentCol = BoardSize / 2
...
function OnClick(e) {
    ...
    // The cell has the id "space" + a number.
    var cellID = elementID.substr(5, elementID.length - 5)
    currentCell = cellID
    currentRow = currentCell / BoardSize;
...
function OnKey(e) {
    ...
    if (keyID >= 65 && keyID <= 90) {
        ...
        if (tileRack.hasChildNodes) {
            ...
            if (found) {
                var boardSpace = document.getElementById("space" + currentCell)
                var newParent = boardSpace
    ...
    function updateBoard() {
        ...
        for (var index = 0; index < tilesOnBoard.length ; index++) {
            ...
            oldRows = oldRows.concat(Math.round(cellId / 15 - 0.5));
            oldCols = oldCols.concat(cellId % 15);
        }
        ...
        for (var index = 0; index < playedTiles.length; index++) {
            ...
            newRows = newRows.concat(Math.round(cellId / 15 - 0.5));
            newCols = newCols.concat(cellId % 15);
        ...
        if (isFirstMove()) {
            ...
            if (direction == 2) {
                ...
                while (!done) {
                    ...
                    col++;
                    if (col > 15 || count > playedTiles.length) {
                        done = true;
                    }
                }
            } else {
